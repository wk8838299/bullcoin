...
{
    // Convert feecoin:// to feecoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("feecoin://"))
    {
        uri.replace(0, 12, "feecoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Feecoin.lnk";
}
...
{
    return GetAutostartDir() / "feecoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Feecoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  feecoin-qt [" + tr("command-line options") + "]                     " + "\n";
